...
class Service{
  ...
  async addInfo(url,title,website,cover){
    ...
    try {
      const [result] = await connections.query(statement, [title, url, website, cover]);
  ...
  async search(text,tableName){
    const statement = `SELECT id,imgName,cover from ${tableName} WHERE imgName LIKE '${'%' + text.replace(/'/g,'') + '%'}'`;
    try {
